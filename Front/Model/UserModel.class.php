...
//            $usercourselearninfo['total']['weightscore'] += $weightscore;
    ...
    public function getUserCourse($userid=null, $courseid=null, $start=0, $length=9999)
    {
        ...
        $result = M('user_course')->alias('a')->join(' __COURSE__ b on a.courseid=b.courseid and b.isshow=1 ')->field('a.*, b.title, b.typeid, b.classid')->where($where)->order('a.completetime desc')->limit($start, $length)->select();
    ...
    //获取党员学习得分排名 前十名
    public function getUserLearninglistOld()
    {
        //测评得分
    ...
    public function getUserLearninglist()
    {
        ...
        //测评得分
        $userscores = M('user')->alias('u')->field('u.userid, u.username, SUM(ut.gotscore) as gotscore, c.typeid')
            ->join(' left join __USER_TESTING__ ut on ut.userid=u.userid and ut.status=1 ')
            ->join(' left join __TESTING__ t on t.testingid=ut.testingid and t.status=1 ')
            ->join(' left join __COURSE__ c on t.courseid=c.courseid and c.isshow=1 ')
            ->where(array('u.status'=>1))
            ->group('u.userid, c.typeid')
            ->order('gotscore desc, u.userid asc')
            ->select();

        //测评总数
        $data = M('testing')->alias('t')->field('COUNT(t.testingid) as testingnum, c.typeid')
            ->join(' left join __COURSE__ c on t.courseid=c.courseid and c.isshow=1 ')
            ->group('c.typeid')
            ->select();
        $testingtotalnum = array();
        if (is_array($data) && !empty($data)) {
            foreach ($data as $d) {
                $testingtotalnum[$d['typeid']] = $d;
            }
        }
        ...
        if (is_array($userscores)&&!empty($userscores)) {
            foreach ($userscores as $d) {
                if (!isset($userlearning[$d['userid']])) {
                    $userlearning[$d['userid']] = array(
                        'userid'    => $d['userid'],
                        'username'  => $d['username'],
                        'gotscore'  => 0,
                    );
                }
                if ($d['typeid']) $userlearning[$d['userid']]['gotscore'] += floor(($d['gotscore']/$testingtotalnum[$d['typeid']]['testingnum'])*$coursetype[$d['typeid']]['weight']);
            }
        }
